	...
	function displayEventList($num) {
		...
		$output = '<ul id="events-calendar-list">
					<li class="clearfix title-row">
						<span class="list-date"><strong>Date</strong></span>
						<span class="list-event-title"><strong>Venue</strong></span>
						<span class="list-location"><strong>Location</strong></span>
						<span class="list-time"><strong>Time</strong></span>
					</li>';
		...
		foreach($events as $event) {
			if ($event->accessLevel == 'public' || $current_user->has_cap($event->accessLevel)) {
				...
				if ($PostID == '')
					$titlinked = '<span class="list-date">'.$day_names.' '.$startDate.'</span>: ' . $event->eventTitle;
				else
					$titlinked = '<a href="' . get_permalink($PostID) . '">'
								  . '<span class="list-date">' . $day_names . ' ' . $startDate . '</span> ' 
								  . '<span class="list-event-title">' . $event->eventTitle . '</span>' 
								  . '<span class="list-location">' . $event->eventLocation . '</span>' 
								  . '<span class="list-time">' . $startTime . '</span>' 
								. '</a>';
				...
				$output .= '<li id="events-calendar-list-' . $event->id . '" class="clearfix">' . $titlinked .'</li>' . "\n";
			}
		}
